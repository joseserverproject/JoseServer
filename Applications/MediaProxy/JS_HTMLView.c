...
void JS_HTMLView_DoCallJavascript(HWND hwnd, const char * strFunctioinName, const char * strArg) 
{
	...
	if (!browserObject->lpVtbl->QueryInterface(browserObject, &IID_IWebBrowser2, (void**)&webBrowser2)) {
		...
		BSTR bufferScript;
		static DWORD size=0;
		char nameInput[2000]={'\0'};

		memset(nameInput,'\0',sizeof(nameInput)-1);

		wsprintf(nameInput,"javascript:%s('%s');",strFunctioinName,strArg);
		...
		webBrowser2->lpVtbl->Navigate(webBrowser2,bstrScript,NULL,NULL,NULL,NULL);
